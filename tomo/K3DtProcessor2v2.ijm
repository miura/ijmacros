...
function linktracks(tempstr) {
	...
	for(i=0; i<trackIDA.length; i++) {
		print("track"+ trackIDA[i]+": "+trackpointsA[i]+" checkfinish Front:"+checkfinishedFrontA[i] +" Back:"+checkfinishedBack[i]+ " endframe="+efA[i]);
		...
		FinMinimumDistTrackID(checkfinishedFrontA, checkfinishedBackA, efA, exA, eyA, ezA, sfA, sxA, syA, szA, nextIDA, nextIDframeA, nextIDdistance, nextIDendframe, foreIDA, foreIDframeA, foreIDdistance, foreIDendframe, connectionNumberA, TempPotentialPairA, fullTrackLength, totaltracknum, i);
...
/*	while (CheckMultipleConnection(connectionNumberA)==1) {
		for(i=0; i<connectionNumberA.length; i++) {
			if (connectionNumberA[i]>1) {
				...
				expectedpairID = nextIDA[i];
				for(j=0; j<nextIDA.length; j++) {
					if ((trackIDA[j] !=expectedpairID) && (nextIDA[j]==currenttrackID) ) {
...
function FinMinimumDistTrackIDOLD(checkfinishedA, efA, exA, eyA, ezA, sfA, sxA, syA, szA, nextIDA, nextIDframeA, nextIDdistance, nextIDendframe, connectionNumberA, fullTrackLength, totaltracknum, i) {
	distance3DA=newArray(totaltracknum);
	distance2DA=newArray(totaltracknum);
	...
	if ((checkfinishedA[i]==0) && (efA[i]<(fullTrackLength-1)) ) {
		...
		for(j=0; j<sfA.length; j++) {
			...
			if ((i!=j) && (checkfinishedA[j]==0) && ( framegap>=0 ) && (framegap<3)) {
				...
				print("         candidate track"+trackIDA[j]+" framegap:"+framegap +":: dist: " + returnDistance3D(exA[i], eyA[i], ezA[i], sxA[j], syA[j], szA[j]));
			}
		}
...
function FinMinimumDistTrackIDFront(checkfinishedFrontA, checkfinishedBackA, efA, exA, eyA, ezA, sfA, sxA, syA, szA, nextIDA, nextIDframeA, nextIDdistance, nextIDendframe, foreIDA, foreIDframeA, foreIDdistance, foreIDendframe, connectionNumberA, TempPotentialPairA, fullTrackLength, totaltracknum, i){
	maxframegap=3;
	distance3DA=newArray(totaltracknum);
	distance2DA=newArray(totaltracknum);
	for(k=0; k<distance3DA.length; k++) distance3DA[k]=10000;
		...
		for(j=0; j<efA.length; j++) {
			if ( (checkfinishedBackA[j]==0) && (i!=j) ) {
				...
				if (( framegap>=0 ) && (framegap<maxframegap)) {
					...
					print("         candidate track (fore)"+trackIDA[j]+" framegap:"+framegap +":: dist: " + returnDistance3Dzscale(exA[j], eyA[j], ezA[j], sxA[i], syA[i], szA[i], 20));
				}
			}
		}
		minarraypos =K_retrunArrayMinPosition(distance3DA);
...
function FinMinimumDistTrackIDBack(checkfinishedFrontA, checkfinishedBackA, efA, exA, eyA, ezA, sfA, sxA, syA, szA, nextIDA, nextIDframeA, nextIDdistance, nextIDendframe, foreIDA, foreIDframeA, foreIDdistance, foreIDendframe, connectionNumberA, TempPotentialPairA, fullTrackLength, totaltracknum, i){
	maxframegap=3;
	distance3DA=newArray(totaltracknum);
	distance2DA=newArray(totaltracknum);
	for(k=0; k<distance3DA.length; k++) distance3DA[k]=10000;
...
//	if ((checkfinishedA[i]==0) && (efA[i]<(fullTrackLength-1)) ) {
	if (checkfinishedBackA[i]==0) {
		print("         "+sxA[i]+", "+syA[i]+", "+szA[i]+" startframe:"+sfA[i]+" endframe:"+efA[i]);
		for(j=0; j<sfA.length; j++) {
			if ((i!=j) && (checkfinishedBackA[j]==0)) {
				framegap = abs(sfA[j] - efA[i]);
				if (( framegap>=0 ) && (framegap<maxframegap)) {
					//distance3DA[j] = returnDistance3D(exA[i], eyA[i], ezA[i], sxA[j], syA[j], szA[j]);
					distance3DA[j] = returnDistance3Dzscale(exA[i], eyA[i], ezA[i], sxA[j], syA[j], szA[j], 20);
					distance2DA[j] = returnDistance2D(exA[i], eyA[i], sxA[j], syA[j]);
					print("         candidate track (back)"+trackIDA[j]+" framegap:"+framegap +":: dist: " + returnDistance3D(exA[i], eyA[i], ezA[i], sxA[j], syA[j], szA[j]));
				}
			}
		}
		minarraypos =K_retrunArrayMinPosition(distance3DA);
...
function CheckBothSidesAndConnect(checkfinishedFrontA, checkfinishedBackA, nextIDA, nextIDframeA, nextIDdistance, nextIDendframe, foreIDA, foreIDframeA, foreIDdistance, foreIDendframe){
	for(i=0; i<checkfinishedBackA.length; i++) {
		if (checkfinishedBackA[i] ==0) {
			for(j = 0; j< checkfinishedFrontA.length[j]; j++) {
				if (checkfinishedFrontA[i] ==0) {
					if (nextIDA[i] == foreIDA[j]) {
						checkfinishedBackA[i] =1;
						checkfinishedFrontA[j] =1;
						print("CONNECTED: track"+foreIDA[j]+"frame" +foreIDframeA[j]+ " to track"+ nextIDA[i])+" frame" +nextIDframeA[i] + "Dist:"+ nextIDendframe[i]); 
					}
				}
			}
		}
