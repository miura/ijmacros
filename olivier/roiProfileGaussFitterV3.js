// Olivier's Measurement tool, muscle fiber. miura@embl.de 
//
// 110415 suppressed NaN error due to 'infinity' slope.
// 110511-3 added Gaussian fitting for the averaged curve. 
// 110520 fit each profile.
// TODO: set some minimum for the goodness of fit, and use the diameter with good quality fitting to estimate the average diameter. 
// WORKFLOW: select a fiber by segmented line ROI tool and then exectute the script. 
// prints out fit result parameters and also estiamted diameter from Gaussian distribution fitting. 
...
roiwidth = 4;//half of the length of tangential ROIto be measured. 
...
for (var i in coordA[0]){
	...
	resAA[i] = GaussFitReturnPara(curprofile, false);
  // here, just pass the parameter back and add diameter and goodness	

  if (i == 0) 
    writetoResultstable(resAA[i], headerA, true);
  else
    writetoResultstable(resAA[i], headerA, false);
...
//	var iter = resMap.entrySet().iterator();
  for (var i = 0; i < hA.length; i++) 
    rt.setValue(hA[i], row, resMap.get(hA[i]));
	//while (iter.hasNext()){
	//	element = iter.next();
